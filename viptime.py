    ...
    for row in reader:
        if row[0] == user_id:
            ...
            if current_time > expiry_date:
                ...
                combo("\n➜ Your access is valid! Time remaining: ", BOLD_YELLOW, delay=0.03, end="")
                combo(str(remaining_time), BOLD_GREEN, delay=0.03)
            return

    if not user_found:
        combo("\n➜ Access denied! You must purchase the file first before continuing.   ", BOLD_RED)
        combo("➜ Contact: @AniiRo   ", BOLD_GREEN)
