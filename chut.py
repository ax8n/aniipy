...
client = TelegramClient("bot_seen", api_id=6, api_hash="eb06d4abfb49dc3eeb1aeb98ae0f581e").start(bot_token=BOT_TOKEN)
...
async def check_membership(event):
    ...
    msg = await event.reply(
        ...
        buttons=[Button.url("Join Here", "https://t.me/addlist/cBuVPGngfsAwZGY9")]
    )
...
async def start_handler(event):
    ...
    msg = await event.reply(
        ...
        "```Give admin to bot to in group to makw bot work without reply or mention commands```"
    )
    await auto_delete(event, msg)
