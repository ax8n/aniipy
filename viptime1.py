...
# Function for smooth printing
def combo(s, color="", delay=0.05, end="\n"):
    """Prints text character by character with color and delay for a typing effect."""
    sys.stdout.write(color)  # Apply color before printing text
    for char in s:
        sys.stdout.write(char)
        sys.stdout.flush()
        time.sleep(delay)
    sys.stdout.write("\033[0m" + end)  # Reset color and end character
    ...
    for row in reader:
        if row[0] == user_id:
            ...
            if current_time > expiry_date:
                ...
                sys.stdout.write(BOLD_GREEN + str(remaining_time) + "\033[0m\r")  # Print in same line
                sys.stdout.flush()
            return
